...
def process_one_store(store, venue_id, venue_name):
    store_id = store.get("StoreID")
    store_name = store.get("StoreName")

    for transaction in store.get("Transactions", []):
        transaction_sent_to_celery_counter.inc()
        process_transaction_task.delay(venue_id, venue_name, store_id, store_name, transaction)
        # call_process_transaction_api.delay(
        #        venue_id, venue_name, store_id, store_name, transaction
        #        )
